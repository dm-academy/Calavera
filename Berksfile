source "https://supermarket.chef.io"

metadata


cookbook 'shared',  path: "./cookbooks/shared"
cookbook 'java7',   path: "./cookbooks/java7"
cookbook 'java8',   path: "./cookbooks/java8"
cookbook 'cerebro', path: "./cookbooks/cerebro"
cookbook 'brazos',  path: "./cookbooks/brazos"
cookbook 'espina',  path: "./cookbooks/espina"
cookbook 'hombros', path: "./cookbooks/hombros"
cookbook 'manos',   path: "./cookbooks/manos"
cookbook 'cara',    path: "./cookbooks/cara"
cookbook 'test',    path: "./cookbooks/test"
cookbook 'localAnt',path: "./cookbooks/localAnt"
cookbook 'base',    path: "./cookbooks/base"
cookbook 'localJenkins', path: "./cookbooks/localJenkins"
cookbook 'localTomcat', path: "./cookbooks/localTomcat"