...
# 
# and checks that the key pair exists in /mnt/public/keys (this directory must be shared across the cluster)
# generates if it does not
# force copies keys to ~/.ssh
# (to reset all keys delete /mnt/public/keys and kitchen/vagrant destroy/up)
# 
...
if [[ $(ls /mnt/public/keys) !=  *id_rsa[^\.]* ]] && \
   ...
   then
       ...
       sudo ssh-keygen -q -t rsa -f "/mnt/public/keys/id_rsa" -P "" \
        -C "*** Host key auto-generated on Vagrant provisioning by node $HOSTNAME on "$(date +%F)" "$(date +%T)" UTC ***"
    #regenerate both
    sudo chown vagrant /mnt/public/keys/*
fi

cp -f /mnt/public/keys/id_rsa* ~/.ssh # copy both to user (vagrant for now) .ssh
